...
char* CreatWifiJson(wifi_ap_record_t* aps, uint16_t count, uint64_t TimeStart,
                    uint64_t TimeEnd, int32_t x = 0, int32_t y = 0) {
  // TODO tijdelijke fix
  if (count > 30) {
    count = 30;//zorg er voor dat het niet crasht door teveel geheugen nodig
  }
  ...
  for (uint16_t i = 0; i < count; i++) {
    ...
    char bssid_str[18];
    sprintf(bssid_str, "%02X:%02X:%02X:%02X:%02X:%02X", aps[i].bssid[0],
    ...
    cJSON_AddNumberToObject(
        ...
        aps[i].he_ap.bss_color); /**< The BSS Color value associated with the
                                    AP's corresponding BSS */
    cJSON_AddNumberToObject(
        network1, "he_ap_partial_bss_color",
        aps[i].he_ap.bss_color); /**< Indicates whether an AID assignment rule
                                    is based on the BSS color */
    cJSON_AddNumberToObject(
        network1, "he_ap_bss_color_disabled",
        aps[i].he_ap.bss_color_disabled); /**< Indicates whether the BSS color
                                             usage is disabled */
    cJSON_AddNumberToObject(
        network1, "he_ap_bssid_index",
        aps[i].he_ap.bssid_index); /**< In a M-BSSID set, identifies the
  ...
  // puts(json_string);// door het te verweideren crasht het niet.
